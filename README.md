...
| **read-ideaweb** | Pull all your [IdeaWeb](https://ideaweb-app.vercel.app) idea-notebooks from the live cloud into Claude as lean Markdown — from **any** project folder, one read-only command instead of exporting a file per project. Optional topic filter. *(Personal skill — repoint the script path to your own IdeaWeb clone; see the `SKILL.md`.)* |
| **research-team** | An on-call **multi-agent research team**: a router scales the crew to your ask, parallel researchers go deep and cite sources, an **adversarial fact-check** cell tries to *refute* every load-bearing claim, a multi-lens advisor panel weighs in, then it synthesizes a report (markdown or an interactive HTML deliverable). Reuses `mckinsey-report` for rigor + house style; runs its fan-out via the Claude Code `Workflow` tool. Self-contained; works for anyone. |
| **loop-check** | A **final-check gate** that loops any finished project (code, plan, skill, report, app) to a binary standard — independent critic agents (no self-review bias) → adversarial verify (kills false positives) → gate → optional `--fix` → re-check — ending in a **READY / READY-WITH-FIXES / NOT-READY** verdict. Pairs with `research-team` (one produces, one gates). Self-contained; works for anyone. |
